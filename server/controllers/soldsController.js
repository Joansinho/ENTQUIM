...
const ventasController = {
    ...
    getVentaById: (req, res) => {
        const { id_venta } = req.params; // Obtenemos el ID de la venta desde los parámetros de la URL
    
        if (!id_venta) {
            return res.status(400).json({ error: 'El id_venta es obligatorio.' });
        }
        ...
        db.query(query, [id_venta], (err, results) => {
            if (err) {
                return res.status(500).json({ error: 'Error al obtener la venta', detalles: err });
            }
    
            if (results.length === 0) {
                return res.status(404).json({ message: 'No se encontró una venta con el ID proporcionado.' });
            }
    
            // Estructuramos la respuesta agrupando los detalles
            const venta = {
