...
const registerByAdmin = async (req, res) => {
    ...
    try {
        ...
        const { insertId } = await new Promise((resolve, reject) => {
            const query = `
                INSERT INTO Usuarios (correo, contrasena, nombre, apellido, id_rol)
                VALUES (?, ?, ?, ?, ?)
            `;
            connection.query(query, [correo, hashedPassword, nombre, apellido, id_rol], (err, results) => {
                if (err) return reject(err);
                resolve(results);
            });
        });
        ...
        res.status(201).json({
            message: "Usuario registrado exitosamente por administrador. Correo enviado.",
            id_usuarios: insertId,
        });
    } catch (err) {
        console.error(err);
        res.status(500).json({ error: "Error en la base de datos o al enviar el correo." });
    }
};
